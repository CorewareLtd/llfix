...
def main(file_path):
    ...
    try:
        ...
        if timestamps:
            ...
            print("Incoming 35=D count : " + str(len(timestamps)) + " (Ignoring first 1% of samples)")
...
if __name__ == "__main__":
    incoming_message_serialisation_path = "clients_incoming"

    if not os.path.isdir(incoming_message_serialisation_path):
        print(f"Error: required directory '{incoming_message_serialisation_path}' does not exist. Run the benchmark first.")
        sys.exit(1)

    messages_file = "incoming.txt"

    if os.path.exists(messages_file):
        os.remove(messages_file)

    command = "./deserialiser -i ./" + incoming_message_serialisation_path + " -o " + messages_file

    if sys.platform.startswith("win"):
        command = ".\\deserialiser.exe -i .\\" + incoming_message_serialisation_path + " -o " + messages_file
    else:
