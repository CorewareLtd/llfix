...
class TestClient : public llfix::FixClient<llfix::TCPConnector>
#else
#include <llfix/core/solarflare_tcpdirect/tcp_connector_tcpdirect.h>
class TestClient : public llfix::FixClient<llfix::TCPConnectorTCPDirect>
#endif
{
    private:
        uint32_t m_order_id = 0;
        std::size_t m_execution_report_count = 0;
        std::size_t m_order_cancel_replace_reject_count = 0;
        std::unordered_map<uint32_t, Order*> m_orders;
        bool m_use_repeating_groups=  true;
    ...
    public:
        ...
        void set_use_repeating_groups (bool b) { m_use_repeating_groups = b; }

        std::size_t execution_report_count() { return m_execution_report_count;  }
        void reset_execution_report_count() { m_execution_report_count = 0; }

        std::size_t order_cancel_replace_reject_count() { return m_order_cancel_replace_reject_count; }

        uint32_t get_order_id() { return m_order_id; }
        ...
        {
            LLFIX_UNUSED(message);
            LLFIX_LOG_INFO("Logout response");
        }
        ...
        {
            LLFIX_LOG_INFO("Server heartbeat");
        }
        ...
        {
            m_order_cancel_replace_reject_count++;

            if (message->has_tag(11))
            {
                auto order_index = message->get_tag_value_as<uint32_t>(11);

                if (order_index >= m_orders.size())
                {
                    LLFIX_LOG_ERROR("Received replace order reject for an unknown order, order id " + std::to_string(order_index));
                    return;
                }
        ...
        void send_new_order(Order* order, bool fake=false)
        {
            ...
            // ACCOUNT
            message->set_tag(1, "abcdefghabcdefghabcdefghabcdefghabcdefghabcdefghabcdefghabcdefghabcdefgh");
            ...
            if(fake)
            {
                send_fake_outgoing_message(message);
            }
            else
            {
        ...
        /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        // HEARTBEAT
        void send_heartbeat(bool fake)
        {
            auto message = outgoing_message_instance();

            // MSG TYPE
            message->set_msg_type('0');
