...
int main(int argc, char* argv[])
{
    ...
    for (int i = 0; i < client_count; i++)
    {
        ...
        fix_session_settings.replay_messages_on_incoming_resend_request = true;
        #endif
        ...
        fix_session_settings.application_dictionary_path="../../dictionaries/FIX44.xml";
        #endif
    ...
    while(true)
    {
        ...
        llfix::Console::print_colour(llfix::ConsoleColour::FG_RED, "Press 5 reconnect a specific client\n");
        ...
        else if (user_input[0] == '5') // RECONNECTION FOR A SPECIFIED CLIENT
        {
            llfix::Console::print_colour(llfix::ConsoleColour::FG_YELLOW, "Please enter client index and press enter :");
            int client_index_to_reconnect{ -1 };
            std::string string_client_index_to_reconnect;
            std::cin >> string_client_index_to_reconnect;

            try
            {
                client_index_to_reconnect = std::stoi(string_client_index_to_reconnect);

                if (client_index_to_reconnect >= client_count || client_index_to_reconnect < 0)
                {
                    llfix::Console::print_colour(llfix::ConsoleColour::FG_YELLOW, "You need to specify a valid client indext\n");
                }
                else
                {
                    Task* new_task = new Task;
                    new_task->task_type = TaskType::RECONNECT;

                    task_queues[client_index_to_reconnect].push(new_task);
                    llfix::Console::print_colour(llfix::ConsoleColour::FG_YELLOW, "Task added to the queue\n");
                }
            }
            catch (...)
            {
                llfix::Console::print_colour(llfix::ConsoleColour::FG_YELLOW, "You need to specify a positive client index\n");
            }
        }
