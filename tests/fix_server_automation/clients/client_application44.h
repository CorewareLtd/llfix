...
class ClientApplication44 : public FIX::Application, public FIX::MessageCracker
{
public:
    ...
    void send_cancel_order(std::size_t orig_order_id)
    {
        try
        {
            ...
            FIX44::OrderCancelRequest cancelOrder(
                FIX::OrigClOrdID(std::to_string(orig_order_id)),   // original order ID
                FIX::ClOrdID(std::to_string(client_order_id)),     // new unique ID
                FIX::Side(FIX::Side_BUY),
                FIX::TransactTime()
...
private:
    ...
    void onMessage(const FIX44::ExecutionReport& report, const FIX::SessionID& sessionID)
    {
