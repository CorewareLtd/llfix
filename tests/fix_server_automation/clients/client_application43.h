...
class ClientApplication43 : public FIX::Application, public FIX::MessageCracker
{
public:
    ...
    void send_new_order()
    {
        try
        {
            ...
            FIX43::NewOrderSingle newOrder(
                FIX::ClOrdID(std::to_string(client_order_id)),
                FIX::HandlInst(FIX::HandlInst_AUTOMATED_EXECUTION_INTERVENTION_OK),
    ...
    void send_replace_order(std::size_t orig_order_id)
    {
        try
        {
            ...
            FIX43::OrderCancelReplaceRequest replaceOrder(
    ...
    void send_cancel_order(std::size_t orig_order_id)
    {
        try
        {
            ...
            FIX43::OrderCancelRequest cancelOrder(
                FIX::OrigClOrdID(std::to_string(orig_order_id)),   // original order ID
                FIX::ClOrdID(std::to_string(client_order_id)),     // new unique ID
                FIX::Side(FIX::Side_BUY),
                FIX::TransactTime()
...
private:
    ...
    void onMessage(const FIX43::ExecutionReport& report, const FIX::SessionID& sessionID)
    {
